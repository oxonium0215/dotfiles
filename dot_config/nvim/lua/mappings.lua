...
M.fzf = {
  ...
  { "n", "<leader>ff", "<cmd>lua require('fzf-lua').files()<CR>", { desc = "fzf-lua find files" } },
  {
    "n",
    "<leader>fa",
    "<cmd>lua require('fzf-lua').files({ no_ignore = true, hidden = true, follow = true })<CR>",
...
M.blankline = {
  {
    "n",
    "<leader>cc",
    "<cmd>lua require('ibl.scope').get(vim.api.nvim_get_current_buf(), { scope = { exclude = { language = {}, node_type = {} }, include = { node_type = {} } } }):range() and vim.api.nvim_win_set_cursor(0, { require('ibl.scope').get(vim.api.nvim_get_current_buf(), { scope = { exclude = { language = {}, node_type = {} }, include = { node_type = {} } } }):range()[1] + 1, 0 })<CR>",
